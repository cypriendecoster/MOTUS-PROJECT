document.addEventListener('DOMContentLoaded', function () {
    ...
    statsDiv.id = 'stats';
    statsDiv.innerHTML = "Historique des parties :<br>Gagnees : 0 | Perdues : 0 | Moyenne d'essais : 0";
    ...
    const wordlist = ['POMME', 'POIRE', 'BANANES', 'ORANGE', 'KIWI', 'FRAISE', 'MANGUES', 'ANANAS', 'RAISIN', 'CERISE'];
    let wordToGuess = [];
    let currentRow = 0;
    let currentCol = 1;
    let foundLetters = [];
    ...
    function updateCursor() {
        ...
        rows.forEach(row => row.querySelectorAll('.cursor').forEach(cursor => cursor.remove()));
    ...
    function updateKeyboard(cells) {
        ...
        cells.forEach(cell => {
            ...
            } else if (bg === 'orange' && keyButton.style.backgroundColor !== 'green') {
                keyButton.style.backgroundColor = 'orange';
                keyButton.style.color = 'white';
            } else if (bg === 'grey' && !['green', 'orange'].includes(keyButton.style.backgroundColor)) {
                keyButton.style.backgroundColor = 'grey';
                keyButton.style.color = 'white';
            }
        });
    }
    ...
    validateBtn.addEventListener('click', function () {
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            if (cells[i].textContent.trim() === '') {
                alert('Vous devez completer toutes les cases avant de valider.');
                return;
            }
        }
        ...
        cells.forEach((cell, index) => {
            ...
            setTimeout(() => {
                if (userLetter === correctLetter) {
                    ...
                    cell.setAttribute('aria-label', `Lettre ${userLetter}, presente mais mal placee`);
                } else {
        ...
        if (userWord === wordToGuess.join('')) {
            ...
            victoryTxt.innerText = 'Vous avez gagne !';
        ...
        if (gameOver) {
            ...
            statsDiv.innerHTML = `Historique des parties :<br>Gagnees : ${wonGames} | Perdues : ${lostGames} | Moyenne d'essais : ${avg}`;
        }
        ...
        setTimeout(() => {
            ...
            const updatedRows = motusTableDiv.querySelectorAll('.ligne');
            if (currentRow < updatedRows.length) {
                const nextCells = updatedRows[currentRow].querySelectorAll('.cell');
    ...
    restartBtn.addEventListener('click', function () {
        ...
        wordToGuess = wordlist[wordIndex].split('');
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            for (let j = 0; j < wordToGuess.length; j++) {
                ...
                if (j === 0 && i === 0) {
                    cell.textContent = wordToGuess[j];
                }
            }
    ...
    deleteBtn.setAttribute('aria-label', 'Supprimer la derniere lettre');
